...
bool GameScene::init()
{
	...
	do 
	{
		...
		int mav=10;
		log("1st 111111111111block %d",mav);
		CC_BREAK_IF(!GameScene::initBoard());
		log("1st 111112222222211block");

	    CC_BREAK_IF(!GameScene::initReadyBubble());
		log("1st 111111333333331block");
		CC_BREAK_IF(!GameScene::initWaitBubble());
		log("1st 11111114444441block");
	...
	do 
	{
		for (int row = 0; row < MAX_ROWS; row++)
		{
			for (int col = 0; col < MAX_COLS - row % 2; col++)
			{
				...
				Bubble* pBubble = this->randomBubble();
				if (pBubble == NULL)
				{
					CC_BREAK_IF(!pBubble);
				}
...
Bubble* GameScene::randomBubble()
{
	...
	do 
	{
		...
		if(pBubble && pBubble->initWithFile2(g_bubbleName[color]))
		{
...
/**************************************************************************************/
//已阅
Bubble* GameScene::createBubble(/*BUBBLE_COLOR*/ int color)
{
	Bubble *pBubble = NULL;
	do 
	{
		...
		if(pBubble && pBubble->initWithFile2(g_bubbleName[color]))// && pBubble->createWithSpriteFrameName(g_bubbleName[color]))
		{
			pBubble->setBubbleColor(color);
		}
		else
...
// 	}
	...
	auto diff=pos-m_curReady->getPosition();
	m_real = diff.getNormalized();
	setDisableEnable();
	this->scheduleUpdate();
...
void GameScene::changeWaitToReady()
{
	...
	m_wait[MAX_WAIT_BUBBLE - 1] =this->randomBubble();
